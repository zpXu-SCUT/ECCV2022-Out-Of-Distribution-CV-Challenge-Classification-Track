...
def main():
    ...
    for nuisance in val_nuisances:
        dataset = ImageDataset("../ROBINv1.1-cls-pose/ROBINv1.1-cls-pose/nuisances/context/Images")
        ...
        scores_logit = pd.concat([scores['imgs'],scores['label'], preds_raw], axis=1)
        scores_logit.to_csv(args.scoreoutput_path+nuisance+'.csv', index=False, header=True)  # 保存logits
        ...
        submit.to_csv(args.output_path+nuisance+'.csv', index=False, header=True)
